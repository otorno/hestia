...
class GaiaPassthroughDriver implements Driver {
  ...
  async init(id: string, config: { page_size: number }, api: DriverApiInterface) {
    this.id = id;
    ...
    return {
      name: 'Gaia Hub Passthrough Driver',
      longId: 'io.github.michaelfedora.hestia.gaiaPassthrough',
      icon,
...
// multi-instance!
export default Object.freeze({ create() { return new GaiaPassthroughDriver(); } });

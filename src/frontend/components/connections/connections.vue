...
<div id='hestia-connections' class='modal-card' style='width: auto'>
  ...
  <section class='modal-card-body'>
    ...
    <div v-for='conn of connections' :key='conn.id' class='conn-entry'>
      <button
        class='button is-danger is-inverted'
        :disabled='connections.length <= 1'
        :title='connections.length <= 1 ? "Cannot remove last connection" : "Remove connection"'
        @click='unregister(conn.id)'
      >
        <b-icon icon='minus-circle' />
      </button>
      <button
        class='button is-inverted'
        :class='{ "is-primary": conn.default, "is-black": !conn.default }'
        :disabled='conn.default || conn.rootOnly'
      ...
      >
        <b-icon :icon='conn.default ? "star" : "star-outline"' />
      </button>
      <img :src='conn.icon'>
      <span>{{ conn.name }}</span>
      <b-icon v-if='conn.rootOnly' icon='folder-alert' style='opacity: 0.5' title='Only stores root directory files (i.e. profile.json)' />
      <b-icon v-if='conn.limitedSpace' icon='folder-alert' class='has-text-danger' style='opacity: 0.5' title='Low on space' />
      <b-icon v-if='conn.noDriver' icon='alert-octogon' style='has-text-danger' title='No driver -- non-useable currently.' />
    ...
    <div v-if='connections.length === 0' class='conn-entry'>
      Nothing here... (minimum of 1 required)
    </div>
    <hr>
    <h2 class='title is-5'>Drivers</h2>
    <div v-for='d of drivers' :key='d.id' class='conn-entry'>
      <button
        class='button is-success is-inverted'
        :disabled='getDriverDisabled(d)'
      ...
      <img :src='d.icon'>
      <span>{{ d.name }}</span>
      <b-icon v-if='d.rootOnly' icon='folder-alert' style='opacity: 0.5' title='Only stores root directory files (i.e. profile.json)' />
    </div>
    <div v-if='drivers.length === 0' class='conn-entry'>
